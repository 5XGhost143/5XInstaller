...
9. Build the Installer with pyinstaller COMMAND: pyinstaller --onefile --noconsole --icon=ico/installer.ico main.py or the .bat (if it's detected as a Virus maybe use a different builder which has noconsole function and onefile and just name it main.exe OR u just let it sign for too much money it should be abt 99€ - 200€)
